...
template<class DATATYPE,
        int TMAXNODES = 8, int TMINNODES = TMAXNODES / 2, int TMAXDATAPOINTS = 256, int TMINDATAPOINTS = TMAXDATAPOINTS / 2>
                ...
                protected:
                    ...
                    bool InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node);
                    ...
                    void ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf);
...
void RTREE_QUAL::Insert_gradual(float a_min[NUMDIMS], float a_max[NUMDIMS], DATATYPE &a_dataId) {
    ...
    InsertRectRecGradually(a_min, a_max, &a_dataId, 1, this_rect, m_root);
}
...
bool RTREE_QUAL::InsertRectRec(const Branch &a_branch, Node *a_node, Node **a_newNode, int a_level, Rect &a_coverRect, Rect &a_newCoverRect) {
    ...
    if (a_node->m_level > a_level) {
        ...
        if(a_level > 0) return AddBranch(&a_branch, a_node, a_newNode, a_coverRect, a_newCoverRect);
            ...
            // I don't think it will ever get here...
            cout << "inserted into leaf, calling ripple_lite, this shouldn't happen ..." << endl;
            exit(3);
        }
    } else {
        // Should never occur
...
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node) {
    ...
    if (a_node->m_level > 0) {
        ...
        if(a_node->m_filled_spare_space + gs_count < SPARE_SPACE_LIMIT){
            ...
            for(int index = 0; index < a_node->m_count; index++){
                InsertRectRecGradually(a_node->m_spare_mins + NUMDIMS*tba_to_children_acc_counts[index], a_node->m_spare_maxes + NUMDIMS*tba_to_children_acc_counts[index], a_node->m_spare_ids + tba_to_children_acc_counts[index], tba_to_children_counts[index], tba_MBRs[index], a_node->m_branch[index].m_child);
            }
        }
        ...
        #endif
        ripple_lite_v2(a_node->m_spare_mins, a_node->m_spare_maxes, a_node->m_spare_ids, a_node->m_filled_spare_space + gs_count, a_node);
        #ifdef timebreak
...
RTREE_TEMPLATE
void RTREE_QUAL::ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf){
