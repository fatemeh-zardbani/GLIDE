...
#define Max std::max
    ...
    clock_t spare_scan_time = 0.0;
    clock_t leaf_scan_time = 0.0;
#endif
...
#define EPSILON 1e-5
    ...
    int fill_holes_count = 0;
    int split_leaf_count = 0;
    int pushed_back_items_count = 0;
#endif
...
int partition_on_one(vector<int> *mapping, int low, int high, int offset)
{   
    // Choosing the pivot
    int folan;
    int pivot = 1;
    ...
    for (int j = low; j <= high - 1; j++) {
        ...
        if ((*mapping)[j - offset] < pivot) {
            ...
            for(folan = 0; folan< NUMDIMS; folan++){
                swap(m_data_arr_mins[i][folan], m_data_arr_mins[j][folan]);
                swap(m_data_arr_maxes[i][folan], m_data_arr_maxes[j][folan]);
            }
            swap(m_data_arr_ids[i], m_data_arr_ids[j]);
            swap((*mapping)[i-offset],(*mapping)[j-offset]);
        }
    }
    for(folan = 0; folan< NUMDIMS; folan++){
        swap(m_data_arr_mins[(i + 1)][folan], m_data_arr_mins[high][folan]);
        swap(m_data_arr_maxes[(i + 1)][folan], m_data_arr_maxes[high][folan]);
    }
    
    swap(m_data_arr_ids[i + 1], m_data_arr_ids[high]);
...
template<class DATATYPE,
        int TMAXNODES = 8, int TMINNODES = TMAXNODES / 2, int TMAXDATAPOINTS = 256, int TMINDATAPOINTS = TMAXDATAPOINTS / 2>
                ...
                public:
                    ...
                    void Insert_gradual(float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId);


                    void Delete(float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId);
                    ...
                    void findTrashInAllPlaces();
                    void findTrashInAllPlacesRec(Node *a_node);
                    int getCountOfHoles();
                    ...
                    void getLeafAreaPlot(string filename);
                ...
                protected:
                    ...
                    bool deleteRec(Node *a_node, float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId);
                    ...
                    bool InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node, vector<pair<Node*, Branch>> *leaves_to_add_later, Rect &a_coverRect);
                    ...
                    void getLeafAreaPlot(ofstream &file, Node *a_node);
                    ...
                    int PickBranch_lite(const Rect *a_rect, vector<Branch> *choices);
                    ...
                    void LoadLeaves(Node *a_nodeA, Node *a_nodeB, PartitionVars *a_parVars);
                    ...
                    bool Cover(Rect *a_rectA, const float a_min[NUMDIMS], const float a_max[NUMDIMS]) const;
                    ...
                    void ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf, vector<pair<Node*, Branch>> *leaves_to_add_later);
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert_gradual(float a_min[NUMDIMS], float a_max[NUMDIMS], DATATYPE &a_dataId) {
    ...
    // }
    // if(a_dataId == 500106){
    //     cout << "in insert_grad for item 500106 " << endl;
    // }
    Rect this_rect = Rect(a_min, a_max);
    vector<pair<Node*, Branch>> lta_later;
    // cout << "1111111" << endl;
    InsertRectRecGradually(a_min, a_max, &a_dataId, 1, this_rect, m_root, &lta_later, this_rect);
    // cout << "2222222" << endl;
...
bool RTREE_QUAL::InsertRectRec(const Branch &a_branch, Node *a_node, Node **a_newNode, int a_level, Rect &a_coverRect, Rect &a_newCoverRect) {
    ...
    if (a_node->m_level > a_level) {
        ...
        if(a_level > 0) return AddBranch(&a_branch, a_node, a_newNode, a_coverRect, a_newCoverRect);
        else{
            // I think this should never happen
            cout << "in insertrectrec, calling ripple lite, should never happen..." << endl;
            exit(5);
        }
    } else {
        // Should never occur
...
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node, vector<pair<Node*, Branch>> *leaves_to_add_later, Rect &a_coverRect) {
    
    if (a_node->m_level > 0) {
        // IS INNER NODE
        if(a_node->m_filled_spare_space + gs_count < SPARE_SPACE_LIMIT){
            // put them in
            // update MBR
            memcpy(a_node->m_spare_mins + a_node->m_filled_spare_space*NUMDIMS, gs_tba_mins, gs_count*NUMDIMS*sizeof(float));
            memcpy(a_node->m_spare_maxes + a_node->m_filled_spare_space*NUMDIMS, gs_tba_maxes, gs_count*NUMDIMS*sizeof(float));
            memcpy(a_node->m_spare_ids + a_node->m_filled_spare_space, gs_tba_ids, gs_count*sizeof(int));
            
            a_node->m_filled_spare_space += gs_count;
            return false;
        }
        else{
            // diffuse
            ...
            float* tmp_mins = (float*)realloc(a_node->m_spare_mins, (gs_count + a_node->m_filled_spare_space)*NUMDIMS*sizeof(float));
            float* tmp_maxes = (float*)realloc(a_node->m_spare_maxes, (gs_count + a_node->m_filled_spare_space)*NUMDIMS*sizeof(float));
            int* tmp_ids = (int*)realloc(a_node->m_spare_ids, (gs_count + a_node->m_filled_spare_space)*sizeof(int));
            ...
            int *tba_map = (int *)calloc(gs_count + a_node->m_filled_spare_space,sizeof(int));



            Rect this_rect;
            Rect* tba_MBRs = (Rect*)malloc(a_node->m_count * sizeof(Rect));

            for(int i = 0; i < gs_count + a_node->m_filled_spare_space; i++){
            ...
            quickSort(a_node->m_spare_mins, a_node->m_spare_maxes, a_node->m_spare_ids, tba_map, 0, gs_count + a_node->m_filled_spare_space-1);
            ...
            for(int index = 0; index < a_node->m_count; index++){
                if(InsertRectRecGradually(a_node->m_spare_mins + NUMDIMS*tba_to_children_acc_counts[index], a_node->m_spare_maxes + NUMDIMS*tba_to_children_acc_counts[index], a_node->m_spare_ids + tba_to_children_acc_counts[index], tba_to_children_counts[index], tba_MBRs[index], a_node->m_branch[index].m_child, leaves_to_add_later, a_coverRect)){
                    a_node->m_branch[index].m_rect = a_coverRect;
        ...
        // else
        // ripple into array spare and tbas
        ...
        #endif
        ripple_lite_v2(a_node->m_spare_mins, a_node->m_spare_maxes, a_node->m_spare_ids, a_node->m_filled_spare_space + gs_count, a_node, leaves_to_add_later);
        #ifdef timebreak
...
RTREE_TEMPLATE
int RTREE_QUAL::PickBranch_lite(const Rect *a_rect, vector<Branch> *choices) {
    //            ASSERT(a_rect && a_node);
    // cout << "in pick branch" << endl;
    bool firstTime = true;
    float increase;
    ...
    for (int index = 0; index < choices->size(); ++index) {
        ...
        Rect *curRect = &(choices->at(index).m_rect);
        // cout << "in loop," << index << " id: " << a_node->m_branch[index].m_child->m_id << " cureRect: (" << curRect->m_min[0] << ", " << curRect->m_min[1] << ") (" << curRect->m_max[0] << ", " << curRect->m_max[1] << ")" << endl;
        area = CalcRectVolume(curRect);
        tempRect = CombineRect(a_rect, curRect);
        increase = CalcRectVolume(&tempRect) - area;
...
void RTREE_QUAL::GetBranches(Node *a_node, const Branch *a_branch, PartitionVars *a_parVars) {
    ...
    for (int index = 0; index < max_children; ++index) {
        ...
        // but I have to include it any way
            a_parVars->m_branchBuf[index] = Branch(NULL, Rect(m_data_arr_mins[a_node->m_L + a_node->m_holes + index], m_data_arr_maxes[a_node->m_L + a_node->m_holes  + index]), m_data_arr_ids[a_node->m_L + a_node->m_holes + index]);
        } else {
...
bool RTREE_QUAL::Search_2026(Node *a_node, Rect *a_rect, int &a_foundCount, LeavesToAdd_v2 *all_lta) {
    ...
    // scan the spares
    for(int i = 0; i < a_node->m_filled_spare_space; i++){
        if(Overlap(a_rect, a_node->m_spare_mins + i*NUMDIMS, a_node->m_spare_maxes + i*NUMDIMS)){
            #ifdef debug
...
void RTREE_QUAL::Add_ltas_v2(LeavesToAdd_v2 *all_lta){
    ...
    Node* this_leaf_older_brother;
    Node* this_leaf_younger_brother;

    Rect trash_rect;

    float* tmp_mins = (float*)malloc(SPARE_SPACE_LIMIT*NUMDIMS*sizeof(float));
    float* tmp_maxes = (float*)malloc(SPARE_SPACE_LIMIT*NUMDIMS*sizeof(float));
    int* tmp_ids = (int*)malloc(SPARE_SPACE_LIMIT*sizeof(int));
    int tmp_count;
    ...
    for(int i = 0; i < all_lta->size(); i++){
        ...
        if((all_lta->at(i).this_leaf)->m_parent != NULL){
            ...
            memcpy(tmp_mins, (all_lta->at(i).this_leaf)->m_spare_mins, (all_lta->at(i).this_leaf)->m_filled_spare_space*NUMDIMS*sizeof(float));
            memcpy(tmp_maxes, (all_lta->at(i).this_leaf)->m_spare_maxes, (all_lta->at(i).this_leaf)->m_filled_spare_space*NUMDIMS*sizeof(float));
            memcpy(tmp_ids, (all_lta->at(i).this_leaf)->m_spare_ids, (all_lta->at(i).this_leaf)->m_filled_spare_space*sizeof(int));
            tmp_count = (all_lta->at(i).this_leaf)->m_filled_spare_space;
            ...
            int choice = 0;
            Rect this_rect;
            ASSERT(tmp_count < SPARE_SPACE_LIMIT);
            for(int t = 0; t < tmp_count; t++){
                this_rect = Rect(tmp_mins+t*NUMDIMS, tmp_maxes+t*NUMDIMS);
                choice = PickBranch_lite(&this_rect, &tba);
                memcpy(tba[choice].m_child->m_spare_mins+tba[choice].m_child->m_filled_spare_space*NUMDIMS, tmp_mins+t*NUMDIMS, NUMDIMS*sizeof(float));
                memcpy(tba[choice].m_child->m_spare_maxes+tba[choice].m_child->m_filled_spare_space*NUMDIMS, tmp_maxes+t*NUMDIMS, NUMDIMS*sizeof(float));
                tba[choice].m_child->m_spare_ids[tba[choice].m_child->m_filled_spare_space] = tmp_ids[t];
                tba[choice].m_child->m_filled_spare_space++;
                tba[choice].m_rect = CombineRect(&tba[choice].m_rect, &this_rect);
            }
...
//                else this_branch.m_child->m_regular = true;
            ...
            int choice = 0;
            Rect this_rect;
            ASSERT(m_root->m_filled_spare_space < SPARE_SPACE_LIMIT);
            for(int t = 0; t < m_root->m_filled_spare_space; t++){
                this_rect = Rect(m_root->m_spare_mins+t*NUMDIMS, m_root->m_spare_maxes+t*NUMDIMS);
                choice = PickBranch_lite(&this_rect, &tba);
                memcpy(tba[choice].m_child->m_spare_mins+tba[choice].m_child->m_filled_spare_space*NUMDIMS, m_root->m_spare_mins+t*NUMDIMS, NUMDIMS*sizeof(float));
                memcpy(tba[choice].m_child->m_spare_maxes+tba[choice].m_child->m_filled_spare_space*NUMDIMS, m_root->m_spare_maxes+t*NUMDIMS, NUMDIMS*sizeof(float));
                tba[choice].m_child->m_spare_ids[tba[choice].m_child->m_filled_spare_space] = m_root->m_spare_ids[t];
                tba[choice].m_child->m_filled_spare_space++;
                tba[choice].m_rect = CombineRect(&tba[choice].m_rect, &this_rect);
            }

            m_root->m_filled_spare_space = 0;
...
RTREE_TEMPLATE
void RTREE_QUAL::ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf, vector<pair<Node*, Branch>> *leaves_to_add_later){
...
void RTREE_QUAL::findDataInAllPlacesRec(Node *a_node, int data_index){
    ...
    if(a_node->IsInternalNode()){
        ...
        for(int j = a_node->m_L + a_node->m_holes; j < a_node->m_R; j++){
            if(m_data_arr_ids[j] == data_index){
                cout << "data was in leaf  " << a_node->m_id << endl;
                for(int jj = 0; jj < NUMDIMS; jj++) cout << m_data_arr_mins[j][jj] << " " << m_data_arr_maxes[j][jj] << " ,";
                cout << endl; 

                if(a_node->m_parent!= NULL){
                    int branch_index = getNodeBranchIndex(a_node);
                    Rect leaf_cover = (a_node->m_parent)->m_branch[branch_index].m_rect;
                    cout << "leaf cover was: " << endl;
                    for(int jj = 0; jj < NUMDIMS; jj++) cout << leaf_cover.m_min[jj] << " " << leaf_cover.m_max[jj] << " ,";
                    cout << endl; 
                }
            }
        }
...
RTREE_TEMPLATE
void RTREE_QUAL::findTrashInAllPlaces(){
    cout << "in trash_finding" << endl;

    findTrashInAllPlacesRec(m_root);
}

RTREE_TEMPLATE
void RTREE_QUAL::findTrashInAllPlacesRec(Node *a_node){
    if(a_node->IsInternalNode()){
        // check the spares
        for(int i = 0; i < a_node->m_filled_spare_space; i++){
            for(int j = 0; j < NUMDIMS; j++){
                if(a_node->m_spare_mins[i*NUMDIMS + j] > a_node->m_spare_maxes[i*NUMDIMS + j]){
                    cout << "trash was in spare of node " << a_node->m_id << " in level: " << a_node->m_level << ". This node has " << a_node->m_filled_spare_space << " spares." << endl;
                    cout << "stored in spare as: " << endl;
                    for(int jj = 0; jj < NUMDIMS; jj++) cout << a_node->m_spare_mins[i*NUMDIMS + jj] << " " << a_node->m_spare_maxes[i*NUMDIMS + jj] << " ,";
                    cout << endl;
                    cout << "bounds of this node: " << endl;
        ...
        for(int i = 0; i < a_node->m_count; i++){
            for(int j = 0; j < NUMDIMS; j++){
                if(a_node->m_branch[i].m_rect.m_min[j] > a_node->m_branch[i].m_rect.m_max[j]){
                    cout << "MBR was trash, for child " << i << " of node " << a_node->m_id << " child id: " << a_node->m_branch[i].m_child->m_id << " at level: " << a_node->m_level << endl;
                    for(int jj = 0; jj < NUMDIMS; jj++) cout << a_node->m_branch[i].m_rect.m_min[jj] << " " << a_node->m_branch[i].m_rect.m_max[jj] << ", ";
                    cout << endl;
                }
            }
            findTrashInAllPlacesRec(a_node->m_branch[i].m_child);
        }
    }
    else{
        // is leaf
        for(int j = a_node->m_L + a_node->m_holes; j < a_node->m_R; j++){
            for(int i = 0; i < NUMDIMS; i++){
                if(m_data_arr_mins[j][i] > m_data_arr_maxes[j][i]){
                    cout << "trash was: " << endl;
                    for(int jj = 0; jj < NUMDIMS; jj++) cout << m_data_arr_mins[j][jj] << " " << m_data_arr_maxes[j][jj] << " ,";
                    cout << endl; 
                    // return true;
...
RTREE_TEMPLATE
void RTREE_QUAL::getLeafAreaPlot(ofstream &file, Node* a_node)
{
  if(a_node->m_level > 1)
  {
    for(int i = 0; i < a_node->m_count;i++)
    {
        ...
        getLeafAreaPlot(file, a_node->m_branch[i].m_child);
    }
  }
  else if(a_node->m_level == 1)
  {
...
void RTREE_QUAL::getLeafArea(string filename)
{
  ...
  getLeafAreaPlot(f, m_root);
  f.close();
}


RTREE_TEMPLATE
void RTREE_QUAL::getLeafAreaPlot(string filename)
{
  ofstream f;
  f.open(filename.c_str()); 
...
RTREE_TEMPLATE
int RTREE_QUAL::getCountOfHoles(){
    int hole_count = 0;
    Node* this_node = m_leftest_leaf;

    while(this_node != NULL){
        hole_count += this_node->m_holes;
        this_node = this_node->m_right_sibling;
    }
    return hole_count;
}
...
RTREE_TEMPLATE
bool RTREE_QUAL::Cover(Rect *a_rectA, const float a_min[NUMDIMS], const float a_max[NUMDIMS]) const {

    for (int index = 0; index < NUMDIMS; ++index) {
        if ((a_rectA->m_min[index] - EPSILON) > a_max[index] || (a_rectA->m_max[index] + EPSILON) < a_max[index] ||
        a_min[index] > (a_rectA->m_max[index] + EPSILON) || a_min[index] < (a_rectA->m_min[index] - EPSILON)) {
            return false;
        }
    }
    return true;
...
RTREE_TEMPLATE
bool RTREE_QUAL::deleteRec(Node* a_node, float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId){
    // look through spares
    #ifdef delete_timebreak
        auto start_time = clock();
    #endif
    for(int i = 0; i < a_node->m_filled_spare_space; i++){
        if(a_node->m_spare_ids[i] == a_dataId){
            // found it
            for(int j = 0; j < NUMDIMS; j++){
                a_node->m_spare_mins[i*NUMDIMS + j] = a_node->m_spare_mins[(a_node->m_filled_spare_space - 1) * NUMDIMS + j];
                a_node->m_spare_maxes[i*NUMDIMS + j] = a_node->m_spare_maxes[(a_node->m_filled_spare_space - 1)*NUMDIMS + j];
            }
            a_node->m_spare_ids[i] = a_node->m_spare_ids[a_node->m_filled_spare_space - 1];
            a_node->m_filled_spare_space--;


            #ifdef delete_timebreak
                spare_scan_time += clock() - start_time;
            #endif
            return true;
        }
    }
    ...
    if(a_node->m_level > 0){
        ...
        for(int i = 0; i < a_node->m_count; i++){
            // for each child
            if(Cover(&(a_node->m_branch[i].m_rect), a_min, a_max)){
                if(deleteRec(a_node->m_branch[i].m_child, a_min, a_max, a_dataId)){
                    return true;
                }
            }
        }
        ...
        // is leaf
            ...
            start_time = clock();
        #endif
        for(int i = a_node->m_L + a_node->m_holes; i < a_node->m_R; i++){
            if(m_data_arr_ids[i] == a_dataId){
                // found it
                // first let's check if this is the last item in the leaf right now and there are no spares
                if(a_node->m_R - a_node->m_L - a_node->m_holes > 1 || (a_node->m_filled_spare_space > 0)){
                    for(int j = 0; j < NUMDIMS; j++){
                        m_data_arr_mins[i][j] = m_data_arr_mins[a_node->m_L + a_node->m_holes][j];
                        m_data_arr_maxes[i][j] = m_data_arr_maxes[a_node->m_L + a_node->m_holes][j];
                    }
                    m_data_arr_ids[i] = m_data_arr_ids[a_node->m_L + a_node->m_holes];
                    a_node->m_holes++;
                }
                else{
                    // this is the last one and there were no spares
                    // if the leaf has no items in the array but has spares, then we wait until the spares overflow and enter the array
                    // another solution would be to insert the spares into the array right here TODO
                    
                    // we can just disconnect this leaf, and then check the tree 
                    Node* parent = a_node->m_parent;
                    DisconnectBranch(a_node->m_parent, getNodeBranchIndex(a_node));
                    FreeNode(a_node);
                    deleteEmptyNodesUp(parent);
                }
                #ifdef delete_timebreak
                    leaf_scan_time += clock() - start_time;
                #endif
                return true;
            }
        }
...
RTREE_TEMPLATE
void RTREE_QUAL::Delete(float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId){
    if(!deleteRec(m_root, a_min, a_max, a_dataId)){
        cout << "item " << a_dataId << " to be deleted not found in tree" << endl;
        exit(5);
    }
}
