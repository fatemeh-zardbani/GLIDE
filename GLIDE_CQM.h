...
int RTREE_QUAL::QueryAdaptive_v14(const float *a_min, const float *a_max) {
    ...
    #endif
    vector<pair<Node*, Branch>> lta_later; // extra leaves created by the cracks upon "sling"
    #ifdef timebreak
        start_time = clock();
    #endif
                ...
                if(this_R - stochastic_crack_index < (stochastic_crack_index - this_L - this_holes)){
                    ...
                    if(this_holes + this_R - stochastic_crack_index >= k){
                        ...
                        for(int i = 0; i < this_tba_count; i++){
                            trash_rect = Rect(tba_mins[this_tba_start + i], tba_maxes[this_tba_start + i]);
                            if(tba_maxes[this_tba_start + i][stochastic_crack_axis] <= stochastic_crack_pivot){
                                // belongs to the left side
                                // start by filling the younger brother's holes
                                if((this_leaf->m_right_sibling)->m_holes > 0){
                                    memcpy(m_data_arr_mins[this_leaf->m_R], tba_mins[this_tba_start + i], NUMDIMS*sizeof(float));
                                    memcpy(m_data_arr_maxes[this_leaf->m_R], tba_maxes[this_tba_start + i], NUMDIMS*sizeof(float));
                                    m_data_arr_ids[this_leaf->m_R] = tba_ids[this_tba_start + i];
                                    ...
                                    memcpy(m_data_arr_mins[this_leaf->m_L + this_leaf->m_holes - 1], tba_mins[this_tba_start + i], NUMDIMS*sizeof(float));
                                    memcpy(m_data_arr_maxes[this_leaf->m_L + this_leaf->m_holes - 1], tba_maxes[this_tba_start + i], NUMDIMS*sizeof(float));
                                    m_data_arr_ids[this_leaf->m_L + this_leaf->m_holes - 1] = tba_ids[this_tba_start + i];
                                ...
                                memcpy(m_data_arr_mins[newNode->m_R], tba_mins[this_tba_start + i], NUMDIMS*sizeof(float));
                                memcpy(m_data_arr_maxes[newNode->m_R], tba_maxes[this_tba_start + i], NUMDIMS*sizeof(float));
                                m_data_arr_ids[newNode->m_R] = tba_ids[this_tba_start + i];
