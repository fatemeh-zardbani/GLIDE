...
template<class DATATYPE,
        int TMAXNODES = 8, int TMINNODES = TMAXNODES / 2, int TMAXDATAPOINTS = 256, int TMINDATAPOINTS = TMAXDATAPOINTS / 2>
                ...
                public:
                    ...
                    /// Insert entry
                    void Insert_full(const float a_min[NUMDIMS], const float a_max[NUMDIMS], const DATATYPE &a_dataId);
                ...
                protected:
                    ...
                    struct Node {
                        ...
                        // for debug, not really taken care of...
                        std::string name;
                        int m_count;                                  ///< Count
                    ...
                    bool ripple_lite(const Branch &a_branch, Node *a_node, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect);
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert_full(const float a_min[NUMDIMS], const float a_max[NUMDIMS], const DATATYPE &a_dataId) {
...
bool RTREE_QUAL::InsertRectRec(const Branch &a_branch, Node *a_node, Node **a_newNode, int a_level, Rect &a_coverRect, Rect &a_newCoverRect) {
    ...
    if (a_node->m_level > a_level) {
        ...
        if(a_level > 0) return AddBranch(&a_branch, a_node, a_newNode, a_coverRect, a_newCoverRect);
            ...
            #endif
            bool cracked = ripple_lite(a_branch, a_node, a_newNode, a_coverRect, a_newCoverRect);
            #ifdef timebreak
...
RTREE_TEMPLATE
bool RTREE_QUAL::ripple_lite(const Branch &a_branch, Node *a_node, Node** a_newNode, Rect &a_coverRect, Rect &a_newCoverRect){
    if(a_node->m_holes > 0){
        #ifdef stats
            count_total_holes--;
        #endif
            ...
            fill_holes_count++;
        #endif
        m_data_arr_ids[a_node->m_L + a_node->m_holes - 1] = a_branch.m_data;
        memcpy(m_data_arr_mins[a_node->m_L + a_node->m_holes - 1], a_branch.m_rect.m_min, NUMDIMS*sizeof(float));
        memcpy(m_data_arr_maxes[a_node->m_L + a_node->m_holes - 1], a_branch.m_rect.m_max, NUMDIMS*sizeof(float));
        a_node->m_holes -= 1;
        a_node->m_count += 1;
        return false;
    }

    if(a_node->m_id == m_rightest_leaf->m_id){
        m_data_arr_ids[a_node->m_R] = a_branch.m_data;
        memcpy(m_data_arr_mins[a_node->m_R], a_branch.m_rect.m_min, NUMDIMS*sizeof(float));
        memcpy(m_data_arr_maxes[a_node->m_R], a_branch.m_rect.m_max, NUMDIMS*sizeof(float));
        a_node->m_R += 1;
        a_node->m_count += 1;
        return false;
    }
    ...
    if(a_node->m_R - a_node->m_L - a_node->m_holes > MOVING_THRESHOLD){

        int branch_index; Rect this_cover;
        if(a_node->m_parent != NULL){
            branch_index = getNodeBranchIndex(a_node);
            this_cover = a_node->m_parent->m_branch[branch_index].m_rect;
        }
        ...
        int this_L = a_node->m_L, this_R = a_node->m_R, this_holes = a_node->m_holes;
        ...
        if(stochastic_crack_index != this_L + this_holes && stochastic_crack_index != this_R){
            ...
            if(stochastic_crack_index - this_L - this_holes > this_R - stochastic_crack_index){
                /// first piece is bigger
                // move the smaller piece
                ...
                *a_newNode = AllocNode();
                ...
                (*a_newNode)->m_L = last_filled_spot_in_arr;
                last_filled_spot_in_arr += (this_R - stochastic_crack_index) + DEFAULT_HOLES;
                (*a_newNode)->m_R = last_filled_spot_in_arr;
                (*a_newNode)->m_holes = DEFAULT_HOLES;
                #ifdef stats
                    count_total_holes+= DEFAULT_HOLES;
                #endif

                a_newCoverRect = r_rect;
                a_coverRect = l_rect;

                a_node->m_R = stochastic_crack_index;
                (a_node->m_right_sibling)->m_holes += (this_R - stochastic_crack_index);
                (a_node->m_right_sibling)->m_L -= (this_R - stochastic_crack_index);
                #ifdef stats
                    count_total_holes += (this_R - stochastic_crack_index);
                #endif

                // assign the a_branch by scrack
                // put it where it belongs, fix mbrs and L and Rs and holes

                if(a_branch.m_rect.m_max[stochastic_crack_axis] <= stochastic_crack_pivot){
                    // belongs to left side
                    memcpy(m_data_arr_mins[stochastic_crack_index], a_branch.m_rect.m_min, NUMDIMS*sizeof(float));
                    memcpy(m_data_arr_maxes[stochastic_crack_index], a_branch.m_rect.m_max, NUMDIMS*sizeof(float));
                    m_data_arr_ids[stochastic_crack_index] = a_branch.m_data;

                    a_node->m_R++;
                    (a_node->m_right_sibling)->m_holes--;
                    (a_node->m_right_sibling)->m_L++;
                    a_coverRect = CombineRect(&a_coverRect, &(a_branch.m_rect));
                    #ifdef stats
                        count_total_holes--;
                    #endif
                }
                    ...
                    memcpy(m_data_arr_mins[last_filled_spot_in_arr], a_branch.m_rect.m_min, NUMDIMS*sizeof(float));
                    memcpy(m_data_arr_maxes[last_filled_spot_in_arr], a_branch.m_rect.m_max, NUMDIMS*sizeof(float));
                    m_data_arr_ids[last_filled_spot_in_arr] = a_branch.m_data;

                    (*a_newNode)->m_R++;
                    a_newCoverRect = CombineRect(&a_newCoverRect, &(a_branch.m_rect));

                }

                // fix the linked list

                (*a_newNode)->m_left_sibling = m_rightest_leaf;
                m_rightest_leaf->m_right_sibling = (*a_newNode);
                (*a_newNode)->m_right_sibling = NULL;
                m_rightest_leaf = (*a_newNode);

            }
            else{
                // first piece is smaller, so we move that one
                ...
                *a_newNode = AllocNode();
                ...
                a_coverRect = r_rect;
                a_newCoverRect = l_rect;

                #ifdef stats
                    count_total_holes -= a_node->m_holes;
                #endif
                a_node->m_holes = (stochastic_crack_index - this_L);
                #ifdef stats
                    count_total_holes += a_node->m_holes;
                #endif
                ...
                if(a_branch.m_rect.m_max[stochastic_crack_axis] <= stochastic_crack_pivot){
                    ...
                    memcpy(m_data_arr_mins[stochastic_crack_index - 1], a_branch.m_rect.m_min, NUMDIMS*sizeof(float));
                    memcpy(m_data_arr_maxes[stochastic_crack_index - 1], a_branch.m_rect.m_max, NUMDIMS*sizeof(float));
                    m_data_arr_ids[stochastic_crack_index - 1] = a_branch.m_data;
            ...
            a_node->m_count = a_node->m_R - a_node->m_L - a_node->m_holes;
            (*a_newNode)->m_count = (*a_newNode)->m_R - (*a_newNode)->m_L - (*a_newNode)->m_holes;
            ...
            #ifdef stats
                pushed_back_items_count += (*a_newNode)->m_count;
            #endif

            return true;
        }
        else{
            // we got unlucky, let's just move to the end
            
            (a_node->m_right_sibling)->m_L = a_node->m_L;
            (a_node->m_right_sibling)->m_holes += (a_node->m_R - a_node->m_L);
            #ifdef stats
                count_total_holes+= (a_node->m_R - a_node->m_L);
            #endif

            int last_filled_spot_in_arr = m_rightest_leaf->m_R;

            memcpy(m_data_arr_mins[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_mins[a_node->m_L + a_node->m_holes], NUMDIMS*(a_node->m_R - a_node->m_L - a_node->m_holes)*sizeof(float));
            memcpy(m_data_arr_maxes[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_maxes[a_node->m_L + a_node->m_holes], NUMDIMS*(a_node->m_R - a_node->m_L - a_node->m_holes)*sizeof(float));
            memcpy(m_data_arr_ids + last_filled_spot_in_arr + DEFAULT_HOLES, m_data_arr_ids + a_node->m_L + a_node->m_holes, (a_node->m_R - a_node->m_L - a_node->m_holes)*sizeof(int));
            int folan = (a_node->m_R - a_node->m_L - a_node->m_holes);
            a_node->m_L = last_filled_spot_in_arr;
            ...
            a_node->m_R = last_filled_spot_in_arr + 1;

            if(a_node->m_id == m_leftest_leaf->m_id){

                (a_node->m_right_sibling)->m_left_sibling = NULL;

                m_leftest_leaf = a_node->m_right_sibling;

                ASSERT((a_node->m_right_sibling)->m_L == 0);
            }
            else{
                (a_node->m_left_sibling)->m_right_sibling = a_node->m_right_sibling;
                (a_node->m_right_sibling)->m_left_sibling = a_node->m_left_sibling;
            }

            a_node->m_left_sibling = m_rightest_leaf;
            m_rightest_leaf->m_right_sibling = a_node;
            a_node->m_right_sibling = NULL;
            a_node->m_holes = DEFAULT_HOLES;
            #ifdef stats
                count_total_holes += DEFAULT_HOLES;
            #endif
            a_node->m_count = a_node->m_R - a_node->m_L - a_node->m_holes;
            #ifdef stats
                pushed_back_items_count += a_node->m_count;
            #endif

            m_rightest_leaf = a_node;
            return false;
        }       
    }
    else{
        // small enough to move to the back of the array
