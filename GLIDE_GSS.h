...
void RTREE_QUAL::SplitNode(Node *a_node, const Branch *a_branch, Node **a_newNode) {
    if(a_node->m_parent != NULL){
        int branch_index = getNodeBranchIndex(a_node);
        Rect rect = (a_node->m_parent)->m_branch[branch_index].m_rect;
    }
...
void RTREE_QUAL::ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf, vector<pair<Node*, Branch>> *leaves_to_add_later){
    ...
    if(this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes > 2*MAXDATAPOINTS && (k < (this_leaf->m_R - this_leaf->m_L))){
        #ifdef stats
        ...
        int this_I = k;
        if(this_I < MAXDATAPOINTS) this_I = MAXDATAPOINTS;
        int percentile = floor(SAMPLE_SIZE*(this_R-this_L-this_I)/ (this_R-this_L-this_holes));
        vector<float> samples;
        for(int ii = 0 ; ii < SAMPLE_SIZE; ii++)
        {
            samples.push_back(m_data_arr_maxes[this_L + this_holes + rand() % (this_R - this_L - this_holes)][stochastic_crack_axis]); 
        }
        sort(samples.begin(), samples.end()); 
        float stochastic_crack_pivot;
        if(percentile > 0)
            stochastic_crack_pivot = samples[percentile - 1];
        else
            stochastic_crack_pivot = samples[percentile];
        ...
        int fake_this_L = this_L + this_holes, fake_this_R = this_R;


        if((stochastic_crack_index - this_L) < (this_R - stochastic_crack_index)){
            // left piece is smaller
            while((stochastic_crack_index - this_L) < this_I){
                // re crack on the right piece/bigger piece
                if((stochastic_crack_index - this_L) < (this_R - stochastic_crack_index)){
                    fake_this_L = stochastic_crack_index;
                    percentile = floor(SAMPLE_SIZE*((this_I - (stochastic_crack_index - this_L - this_holes)))/ (fake_this_R-fake_this_L));
                }

                ASSERT(percentile >= 0 && percentile <= SAMPLE_SIZE);
                
                for(int ii = 0 ; ii < SAMPLE_SIZE; ii++)
                {
                    samples[ii] = m_data_arr_maxes[fake_this_L + rand() % (fake_this_R - fake_this_L)][stochastic_crack_axis]; 
                }
                sort(samples.begin(), samples.end()); 

                if(percentile != 0) stochastic_crack_pivot = samples[percentile - 1];
                else stochastic_crack_pivot = samples[percentile];

                r_rect.m_min[stochastic_crack_axis] = 21474836;

                stochastic_crack_index = CrackOnAxisCompMax_v45( fake_this_L, fake_this_R, stochastic_crack_pivot,
                                                            stochastic_crack_axis, &l_rect, &r_rect); 

            }
        }
        else{
            // right piece is smaller
            while((this_R - stochastic_crack_index + this_holes) < this_I){
                // crack on the left piece

                if((stochastic_crack_index - this_L) >= (this_R - stochastic_crack_index)){
                    fake_this_R = stochastic_crack_index;
                    percentile = floor(SAMPLE_SIZE*(fake_this_R-fake_this_L-(this_I - (this_R - stochastic_crack_index)))/ (fake_this_R-fake_this_L));
                }
    ...
    else if(this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes > MAXDATAPOINTS && (k < (this_leaf->m_R - this_leaf->m_L))){
        // medicore crack
        // crack
        // the new node that we will be creating
        Rect trash_rect;
